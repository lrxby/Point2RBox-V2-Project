_base_ = [
    '../_base_/datasets/dota.py', '../_base_/schedules/schedule_1x.py',
    '../_base_/default_runtime.py'
]
angle_version = 'le90'

randomness = dict(seed=0, deterministic=False)

# model settings
model = dict(
    type='Point2RBoxV2',
    # 移除了 ss_prob, copy_paste 等不再使用的参数
    data_preprocessor=dict(
        type='mmdet.DetDataPreprocessor',
        mean=[123.675, 116.28, 103.53],
        std=[58.395, 57.12, 57.375],
        bgr_to_rgb=True,
        pad_size_divisor=32,
        boxtype2tensor=False),
    backbone=dict(
        type='mmdet.ResNet',
        depth=50,
        num_stages=4,
        out_indices=(1, 2, 3),
        frozen_stages=1,
        norm_cfg=dict(type='BN', requires_grad=True),
        norm_eval=True,
        style='pytorch',
        init_cfg=dict(type='Pretrained', checkpoint='torchvision://resnet50')),
    neck=dict(
        type='mmdet.FPN',
        in_channels=[512, 1024, 2048],
        out_channels=128,
        start_level=0,
        add_extra_convs='on_output',
        num_outs=3,
        relu_before_extra_convs=True),
    bbox_head=dict(
        type='Point2RBoxV2Head',
        num_classes=15,
        in_channels=128,
        feat_channels=128,
        strides=[8],
        
        # [核心] 使用统计出的先验尺寸 (短边, 长边)
        # 你的统计结果是 27.6, 65.7，这里取整使用
        prior_box_size=[28.0, 66.0], 

        angle_coder=dict(
            type='PSCCoder',
            angle_version='le90',
            dual_freq=False,
            num_step=3,
            thr_mod=0),
        
        # [Loss 1] 分类 (多尺度融合后)
        loss_cls=dict(
            type='mmdet.FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=1.0),
        
        # [Loss 2] 回归 (基于 Prior Box 微调)
        # 权重设为 5.0，既保证回归力度，又不至于梯度爆炸
        loss_bbox=dict(type='GDLoss', loss_type='gwd', loss_weight=5.0),
        
        # [Loss 3 & 4] 几何约束 (辅助)
        loss_naoa=dict(
            type='NAOALoss',
            loss_weight=1.0,
            k_radius=3.0,
            score_alpha=1.0,
            target_classes=None
        ),
        loss_perspective=dict(
            type='PerspectiveAwareSizeConsistencyLoss', 
            loss_weight=0.5,
            beta=1.0,
            norm_type='z-score',
            target_classes=None),
            
        # 移除了 loss_overlap, loss_voronoi, loss_bbox_edg, loss_ss
    ),
    train_cfg=None,
    test_cfg=dict(
        nms_pre=2000,
        min_bbox_size=0,
        score_thr=0.05,
        nms=dict(type='nms_rotated', iou_threshold=0.1),
        max_per_img=2000))

# load point annotations
# 保持原有的增强策略，不做改动
train_pipeline = [
    dict(type='mmdet.LoadImageFromFile', backend_args={{_base_.backend_args}}),
    dict(type='mmdet.LoadAnnotations', with_bbox=True, box_type='qbox'),
    dict(type='ConvertBoxType', box_type_mapping=dict(gt_bboxes='rbox')),
    dict(type='ConvertWeakSupervision', point_proportion=1., hbox_proportion=0),
    dict(type='mmdet.Resize', scale=(1024, 1024), keep_ratio=True),
    dict(
        type='mmdet.RandomFlip',
        prob=0.75,
        direction=['horizontal', 'vertical', 'diagonal']),
    dict(type='mmdet.PackDetInputs')
]

train_dataloader = dict(batch_size=2,
                        dataset=dict(pipeline=train_pipeline))

# optimizer
optim_wrapper = dict(
    optimizer=dict(
        _delete_=True,
        type='AdamW',
        lr=0.00005,
        betas=(0.9, 0.999),
        weight_decay=0.05))

train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=12, val_interval=1)

# [核心] 关闭绘图，防止验证时卡顿
default_hooks = dict(
    visualization=dict(type='mmdet.DetVisualizationHook', draw=False)
)
custom_hooks = [dict(type='mmdet.SetEpochInfoHook')]