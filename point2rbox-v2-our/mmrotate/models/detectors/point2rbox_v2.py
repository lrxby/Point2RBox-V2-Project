...
#         # 3. 【修改核心】显式调用 loss_by_feat 并传入特征图 'feat'
#         # -------------------------------------------------------------------------
#         # 手动 Forward 获取输出
#         outs = self.bbox_head(feat)
        
#         # 解包 batch_data_samples_all，获取 GT 和 Metas
#         batch_gt_instances_all, batch_gt_instances_ignore_all, batch_img_metas_all = unpack_gt_instances(batch_data_samples_all)
        
#         # 显式调用 loss_by_feat，将 'feat' 作为第一个参数传入
#         # outs[0]: cls_scores, outs[1]: bbox_preds, outs[2]: angle_preds
#         losses = self.bbox_head.loss_by_feat(
#             feat, 
#             outs[0], 
#             outs[1], 
#             outs[2],
#             batch_gt_instances=batch_gt_instances_all,
#             batch_img_metas=batch_img_metas_all,
#             batch_gt_instances_ignore=batch_gt_instances_ignore_all
#         )
#         # -------------------------------------------------------------------------
...
@MODELS.register_module()
class Point2RBoxV2(SingleStageDetector):
    """Implementation of `Point2RBox-v2` tailored for Box-Sensitive Learning."""
    ...
    def rotate_crop(
            ...
            batch_gt_instances: InstanceList = None,
            padding: str = 'reflection') -> Tuple[Tensor, InstanceList]:
        """Data Augmentation: Rotate and Crop."""
        device = batch_inputs.device
        n, c, h, w = batch_inputs.shape
        size_h, size_w = size
        crop_h = (h - size_h) // 2
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # 2. 获取预测结果
        # 注意：在 ROI 模式下，predict 返回的是 NMS 后的全图结果，而非一一对应的正样本
        results_list = self.bbox_head.predict(feat, batch_data_samples_all)
        
        # 【修改点】：彻底移除原本的“伪标签反向更新”逻辑
        # 因为 results_list 包含 NMS 后的检测结果，数量与 GT 点不匹配，无法直接赋值
        # 且在 ROI Loss 驱动下，我们不需要显式更新 GT 点的位置
        # Update point annotations logic REMOVED.
