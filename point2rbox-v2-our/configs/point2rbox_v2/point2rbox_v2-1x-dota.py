...
model = dict(
    ...
    bbox_head=dict(
        ...
        angle_coder=dict(
            ...
            num_step=3,
            thr_mod=0),
        # 【修改点 1】：将 loss_cls 改为 CrossEntropyLoss，适配 ROI Classification
        loss_cls=dict(
        ...
        # 【修改点 2】：置零不需要的 Loss
        loss_overlap=dict(
            type='GaussianOverlapLoss', loss_weight=0.0, lamb=0),
        loss_voronoi=dict(
            type='VoronoiWatershedLoss', loss_weight=5.0),
        loss_bbox_edg=dict(
            type='EdgeLoss', loss_weight=0.0), # 置零
        loss_ss=dict(
            type='Point2RBoxV2ConsistencyLoss', loss_weight=0.0), # 置零
        # 【修改点 3】：确保新加的 Loss 开启
        loss_naoa=dict(
            type='NAOALoss',
