...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 # [修改] 参数名改为 loss_perspective 以匹配配置文件
    ...
    def loss_by_feat(
        ...
        # [修改] 获取新 Loss 开关
        calc_perspective = self.loss_perspective.loss_weight > 0
        # -----------------------------------------------------------
        ...
        if len(pos_inds) > 0:
            ...
            # [新增] 计算透视感知尺寸一致性损失
            # 只有当权重 > 0 时才计算，否则直接跳过，loss_perspective_value 保持为 None
            if calc_perspective:
                loss_perspective_value = self.loss_perspective(
                    pos_rbox_preds,
                    pos_cls_scores,
                    pos_labels
                )
            ...
            # [新增] 新 Loss 的空值处理：只在开启时产生 dummy loss
            if calc_perspective: loss_perspective_value = pos_bbox_preds.sum()
        ...
        # [新增] 添加新 Loss 到返回字典
        if loss_perspective_value is not None:
            losses['loss_perspective'] = loss_perspective_value
